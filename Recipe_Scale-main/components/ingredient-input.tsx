...
import additionalIngredientNames from '@/lib/ingredient-names.json'
...
// Merge additional (localized) ingredient names from lib/ingredient-names.json
const LOCALIZED_INGREDIENTS: string[] = Array.isArray(additionalIngredientNames)
  ? additionalIngredientNames.filter((i) => typeof i === 'string')
  : []
...
export function IngredientInput({
  ...
  // Combine previous ingredients with localized + common ingredients (previous first)
  const allSuggestions = useMemo(() => {
    ...
    // localized names should appear before the default common ingredients
    const localized = LOCALIZED_INGREDIENTS.filter(
      (ing) => !previousSet.has(ing.toLowerCase())
    )

    const remaining = COMMON_INGREDIENTS.filter(
      ing => !previousSet.has(ing.toLowerCase()) && !localized.includes(ing)
    )

    return [...uniquePrevious, ...localized, ...remaining]
  }, [previousIngredients])
